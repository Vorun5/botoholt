...
const SongCard = ({ song }: { song: SongPlaying }) => {
  ...
  return (
    <div className={styles.container}>
      <img
        ...
        src={song.isPlaying ? emote : "/emotes/PoroSad.png"}
        alt={"Dance emote"}
        width="120px"
        height="120px"
      />
          ...
          <div className={styles.song__by}>
            <div>
              <div className={styles.by__label}>
                <img
                  className={styles.by__icon}
                  src="/emotes/peepoDJ.gif"
                  alt="peepoDJ"
                />
                {t("song-card.by")}
                <span className={styles.by__name}>{song.nowPlayingOwner}</span>
              </div>
            </div>
            <div className={styles.song__duration}>
              {`${Math.floor(song.nowPlayingDuration! / 60)}${t("minutes")} 
              ${song.nowPlayingDuration! % 60}${t("seconds")}`}
            </div>
          </div>
        </div>
      )}
