...
const SocialMedias = ({socialMedias}: SocialMediasProps) => {
    ...
    return (
        <div className={styles.container}>
            <div className={`${styles.arrow} ${styles.arrow__left} ${disableLeftArrow ? styles.arrow_disable : ""}`}
                 onClick={() => {
                     if (!disableLeftArrow) setScrollIndex(scrollIndex + 1);
                 }}>
            ...
            <div className={styles.social_medias}>
                {socialMedias.map((socialMedia, index) => {
                        ...
                        return (
                            <div key={index} className={styles.social_media__container}
                                 style={{left: `${(index * (25)) + (scrollIndex * 25)}%`}}>
                                <Postel
                                    title={socialMedia.title}
                                    content={<div className={styles.tooltip}><span>{socialMedia.title}</span></div>}>
                                    <a
            ...
            <div className={`${styles.arrow} ${styles.arrow__right} ${disableRightArrow ? styles.arrow_disable : ""}`}
                 onClick={() => {
                     if (!disableRightArrow) setScrollIndex(scrollIndex - 1);
                 }}>
